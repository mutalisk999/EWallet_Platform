...
func (agent *UBAgent) Type() string {
	return "UBAgent"
}

func (agent *UBAgent) CoinType() string {
	return "UB"
}

func (agent *UBAgent) Init(urlstr string) {
	agent.ServerUrl = urlstr
}

func (agent *UBAgent) DoHttpJsonRpcCallType1(method string, args ...interface{}) (*jsonrpc.RPCResponse, error) {
...
func (agent *UBAgent) GetBalanceByAddressRPC(addr string) (string, error) {
...
func (agent *UBAgent) GetUtxosByAddressRPC(addr string) ([]UTXODetail, error) {
	supportCoin, ok := config.GlobalSupportCoinMgr[agent.CoinType()]
	if !ok {
...
func (agent *UBAgent) ImportAddressRPC(address string) error {
...
func (agent *UBAgent) BroadcastTransactionRPC(rawtrx string) (string, error) {
...
func (agent *UBAgent) IsTransactionConfirmedRPC(trxId string) (bool, error) {
...
func (agent *UBAgent) IsAddressValidRPC(address string) (bool, error) {
...
func UBGetUnCompressPubKey(pubKeyBytes []byte) ([]byte, error) {
...
func UBGetCompressPubKey(pubKeyBytes []byte) ([]byte, error) {
	if len(pubKeyBytes) != 64 {
		return nil, errors.New("invalid pubKeyBytes size")
	}
...
func UBCalcAddressByPubKey(pubKeyStr string) (string, error) {
	pubKeyBytes, err := hex.DecodeString(pubKeyStr)
	if err != nil {
		return "", err
	}

	pubkeyCompress, err := UBGetCompressPubKey(pubKeyBytes)
	if err != nil {
		return "", err
	}
...
func (agent *UBAgent) BuildTrxInPutsOutPutsRPC(addrFromStr string, addrToStr string, amountTransferStr string, feeRateStr string) (string, InPuts, OutPuts, error) {
	supportCoin, ok := config.GlobalSupportCoinMgr[agent.CoinType()]
	if !ok {
...
func (agent *UBAgent) CreateRawTransactionRPC(inputs InPuts, outputs OutPuts) (string, error) {
...
func UBUnPackRawTransaction(rawTrx string) (*transaction.Transaction, error) {
...
func UBPackRawTransaction(trxSig transaction.Transaction) (string, error) {
...
func UBCombineSignatureAndPubKey(signature []byte, pubKey []byte) []byte {
...
func (agent *UBAgent) SignRawTransaction(rawTrx string, pubKeyStr string, keyIndex uint16, utxos []UTXODetail) (string, error) {
	pubKeyBytes, err := hex.DecodeString(pubKeyStr)
	if err != nil {
		return "", err
	}
	...
	trx, err := UBUnPackRawTransaction(rawTrx)
	if err != nil {
		return "", err
	}
	...
	for i := 0; i < len(trx.Vin); i++ {
		trxTemp, err := UBUnPackRawTransaction(rawTrx)
		if err != nil {
			return "", err
		}
		...
		rawTrxWithScript, err := UBPackRawTransaction(*trxTemp)
		if err != nil {
			return "", err
		}
		...
		// append SIGHASH_ALL
		rawTrxBytes = append(rawTrxBytes, []byte{0x1 | 0x8, 0x0, 0x0, 0x0}...)

		// append ub
		rawTrxBytes = append(rawTrxBytes, []byte{0x2, 'u', 'b'}...)
		...
		// append SIGHASH_ALL
		signedData = append(signedData, 0x1|0x8)

		scriptSig := UBCombineSignatureAndPubKey(signedData, pubkeyCompress)
	...
	trxSigStr, err := UBPackRawTransaction(*trx)
	if err != nil {
		return "", err
	}
