...
type BTCAgent struct {
	ServerUrl string
}

func (agent *BTCAgent) Type() string {
	return "BTCAgent"
}

func (agent *BTCAgent) CoinType() string {
	return "BTC"
}

func (agent *BTCAgent) Init(urlstr string) {
	agent.ServerUrl = urlstr
}

func (agent *BTCAgent) DoHttpJsonRpcCallType1(method string, args ...interface{}) (*jsonrpc.RPCResponse, error) {
...
func (agent *BTCAgent) GetBalanceByAddressRPC(addr string) (string, error) {
...
func (agent *BTCAgent) GetUtxosByAddressRPC(addr string) ([]UTXODetail, error) {
	supportCoin, ok := config.GlobalSupportCoinMgr[agent.CoinType()]
	if !ok {
...
func (agent *BTCAgent) ImportAddressRPC(address string) error {
...
func (agent *BTCAgent) BroadcastTransactionRPC(rawtrx string) (string, error) {
...
func (agent *BTCAgent) IsTransactionConfirmedRPC(trxId string) (bool, error) {
...
func (agent *BTCAgent) IsAddressValidRPC(address string) (bool, error) {
...
func BTCGetUnCompressPubKey(pubKeyBytes []byte) ([]byte, error) {
...
func (agent *BTCAgent) BuildTrxInPutsOutPutsRPC(addrFromStr string, addrToStr string, amountTransferStr string, feeRateStr string) (string, InPuts, OutPuts, error) {
	supportCoin, ok := config.GlobalSupportCoinMgr[agent.CoinType()]
	if !ok {
...
func (agent *BTCAgent) CreateRawTransactionRPC(inputs InPuts, outputs OutPuts) (string, error) {
...
func (agent *BTCAgent) SignRawTransaction(rawTrx string, pubKeyStr string, keyIndex uint16, utxos []UTXODetail) (string, error) {
	pubKeyBytes, err := hex.DecodeString(pubKeyStr)
	if err != nil {
		return "", err
	}
